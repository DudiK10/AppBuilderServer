<?php
// backend/api/delete_service.php — השבתת שירות (אדמין כל שירות; עובד — שלו בלבד)
...
// Workers can only delete their own services
if ($user['role'] === 'worker') {
    try {
        ...
        if (!$svc || (int)($svc['worker_id'] ?? 0) !== $user['id']) {
            http_response_code(403);
            echo json_encode(['status' => 'error', 'message' => 'אין הרשאה למחוק שירות זה'], JSON_UNESCAPED_UNICODE);
            exit;
        }
    } catch (PDOException $e) {
...
try {
    $stmt = $conn->prepare('UPDATE services SET is_active = 0 WHERE id = :id');
    $stmt->execute([':id' => $serviceId]);

    if ($stmt->rowCount() > 0) {
        echo json_encode(['status' => 'success', 'message' => 'השירות הוסר בהצלחה'], JSON_UNESCAPED_UNICODE);
    } else {
        http_response_code(404);
        echo json_encode(['status' => 'error', 'message' => 'לא נמצא שירות למחיקה'], JSON_UNESCAPED_UNICODE);
    }
} catch (PDOException $e) {
    http_response_code(500);
